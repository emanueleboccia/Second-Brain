...
| `testo_extra.py` | Le scritte delle storie extra del 24/09/2026 nello stesso format: una frase in alto con una parola sola in Niconne giallo, allineata sulla linea di base, e in basso il nome dell'evento o il fatto con la data. Ogni storia è una funzione; per un testo nuovo se ne aggiunge una. |
| `testo_frasi.py` e `monta_frasi.py` | Il format **«una frase per clip»**, nato il 25/09/2026 per il giorno prima dell'apertura: ogni clip ha la sua frase in alto, su due righe con l'ultima parola in Niconne giallo, e in basso resta fermo il nome dell'evento con la data. Le clip cambiano sul battito, otto battiti l'una, e la musica è una traccia sola che prosegue da una storia all'altra. Legge una ricetta con la traccia, l'inizio, il passo e per ogni storia le clip con `file`, `ss`, `cx` e `frase`: quella del 25/09 sta sull'SSD in `3-in-produzione/storie-clip/frasi-2025/`. La prima riga si scrive corta: se è più lunga della seconda si rimpicciolisce, e si vede. |
| `monta_singole.py` | Le storie da una clip sola, senza scritte: una giornata alla volta, da una ricetta con la traccia, la durata e le clip. La musica della giornata si porta a −14 LUFS tutta insieme e poi si taglia, così prosegue da una storia all'altra senza salti di volume. Nato il 24/09/2026. |
| `inventario.py` e `fogli.py` | Il provino di un archivio intero: per ogni clip durata, spazio colore e tre fotogrammi piccoli, un file alla volta con `nice`, poi i fogli a due colonne da guardare. Nati il 24/09/2026 sulle 93 clip del 2025 di Zucche, tra Drive e SSD. Su Drive il collo di bottiglia è lo scaricamento: conviene far leggere prima i file con `cat` in parallelo, che non pesa sul processore. |
